...
//@Repository => 안해도 되긴 함
// JpaRepository -> @Repository가 있음 -> IoC 됨
public interface UserRepository extends JpaRepository<User, Long> {

    // 쿼리 메서드 네이밍
    // - findBy: 조회 시작
    // - Username: 엔티티의 username 필드명 일치
    // - Optional<User> : 결과가 없을 수도 있으므로 Optional 반환
...
//    Optional<User> findByUsernameAndPassword(String username, String password);
    ...
    /**
     * 로그인 시
     * 역할 (ROLE) 정보까지 함께 조회되는 메서드
     * - 세션에 저장된 User 객체에서 isAdmin(), getRoleDisplay() 등을 바로 사용가능하도록 하기 위함
     */
